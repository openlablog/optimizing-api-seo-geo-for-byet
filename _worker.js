export default {
    async fetch(request, env, ctx) {
        try {
            ...
            // InfinityFree免费主机User-Agent要带Language才能支持curl请求
            if (!userAgent.includes("Language/")) {
                userAgent += " Language/zh_CN";
            }

            // InfinityFree免费主机中转页面最多支持三次请求
            // 第一次请求
            let newRequest = cloneRequest(url.href, request.clone(), userAgent, cookie);
            ...
            if (getResponse.type === "json") {
                ...
                // 第二次请求
                let newRequest2 = cloneRequest(getResponse.href, request.clone(), userAgent, getResponse.cookie + "; " + cookie);
                ...
                if (getResponse2.type === "json") {
                    ...
                    // 第三次请求
                    let newRequest3 = cloneRequest(getResponse2.href, request.clone(), userAgent, getResponse2.cookie + "; " + cookie);
                    ...
                    if (getResponse3.type === "json") {
                        ...
                        return new Response("本网站需要Javascript才能运行，请在浏览器中启用Javascript或使用支持Javascript的浏览器", {
                            status: 200
                        });
                    }

                    return getResponse3.responseObject;
                }

                return getResponse2.responseObject;
            }

            return getResponse.responseObject; // 如果出错删除此行
        } catch (e) {
        }
...
function cloneRequest(request_url, request_clone, user_agent = "", cookie = "") {
    ...
    // 修改User-Agent
    if (user_agent !== "") {
        new_request.headers.set("User-Agent", user_agent);
    }
...
async function cloneResponse(response_clone, backendHost, frontendHost, cookie = "") {
    ...
    if (response_clone.status >= 300 && response_clone.status < 400) {
        ...
        if (
            ...
            if (text.match(/a=toNumbers\(".*b=toNumbers\(".*c=toNumbers\(".*max-age=.*location\.href="/g)) {
                // 提取slowAES变量
                let a_match = text.match(/(?<=a=toNumbers\(")([a-fA-F0-9]+)(?="\))/g).toString();
                let b_match = text.match(/(?<=b=toNumbers\(")([a-fA-F0-9]+)(?="\))/g).toString();
                let c_match = text.match(/(?<=c=toNumbers\(")([a-fA-F0-9]+)(?="\))/g).toString();
                let cookie_match = text.match(/max-age=([^"]+)(?=")/g).toString();
                let url_match = text.match(/(?<=location\.href=")([^"]+)(?=")/g).toString();

                // 使用slowAES解密
                let code = toHex(slowAES.decrypt(toNumbers(c_match), 2, toNumbers(a_match), toNumbers(b_match)));

                // 直接返回解密内容
                return {
                    "type": "json",
                    "href": url_match,
                    "cookie": "__test=" + code + "; " + cookie_match,
                };
            } else {
    ...
    // 响应头添加新cookie，下次访问直接跳过中转页面
    if (cookie !== "") {
        new_response.headers.append("Set-Cookie", cookie);
    }

    // 返回Response对象
    return {
        "type": "object",
        "responseObject": new_response,
    };
}
